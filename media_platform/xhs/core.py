...
class XiaoHongShuCrawler(AbstractCrawler):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    utils.logger.info(f"[XiaoHongShuCrawler.search] Sleep random after page {page-1}")
    ...
    async def get_note_detail_async_task(
        ...
        async with semaphore:
            try:
                ...
                await asyncio.sleep(random.uniform(config.CRAWLER_MIN_SLEEP_SEC, config.CRAWLER_MAX_SLEEP_SEC))
                utils.logger.info(f"[get_note_detail_async_task] Sleep random after fetching note {note_id}")
    ...
    async def get_comments(self, note_id: str, xsec_token: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            utils.logger.info(f"[XiaoHongShuCrawler.get_comments] Sleeping for {crawl_interval:.2f}s after fetching comments for note {note_id}")
